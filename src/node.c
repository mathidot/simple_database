...
{
    printf("ROW_SIZE: %d\n", ROW_SIZE);
    printf("COMMON_NODE_HEAD_SIZE: %d\n", COMMON_NODE_HEADER_SIZE);
...
Cursor* internal_node_find(Table *table, uint32_t page_num, uint32_t key)
{
    ...
    uint32_t child_page_index = internal_node_find_child(node,key);
    uint32_t child_page_num = *internal_node_child(node,child_page_index);
...
{
    /*
    Handle spliting the root
    Old root copied to the new page
